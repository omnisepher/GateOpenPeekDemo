...
void AMyProjectCharacter::Tick(float DeltaSeconds) {
	...
	if (IsInteracting) {
		...
		// GEngine->AddOnScreenDebugMessage(-1, 1.f, FColor::Red, FString::SanitizeFloat(InteractionTimer));
		if (InteractionTimer >= 3.0f && InteractionTimer < InteractionTimer+DeltaSeconds) {
			Interact();
		}
	}
...
void AMyProjectCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent) {
	...
	PlayerInputComponent->BindAction("Interact", IE_Released, this, &AMyProjectCharacter::CutInteractTimer);

	// PlayerInputComponent->BindAxis("Use", this, &AMyProjectCharacter::Use);
...
void AMyProjectCharacter::Ray() {
	FVector start = FP_Gun->GetComponentLocation() + FVector(0, 0, 5.f);
	FVector end = FP_MuzzleLocation->GetComponentLocation() + FP_MuzzleLocation->GetRightVector() * 150.0f +
		FVector(0, 0, 50.f);
	FHitResult hit;
	FCollisionQueryParams CollisionQueryParams;
	CollisionQueryParams.AddIgnoredActor(this);
	if (GetWorld()) {
		bool actorHit = GetWorld()->LineTraceSingleByChannel(hit, start, end, ECC_Pawn, CollisionQueryParams);
		// DrawDebugLine(GetWorld(), start, end, FColor::Red, false, 0.1f, 0.f, 1.f);
		if (actorHit && Cast<APawn>(hit.GetActor())) {
			hitRecord = hit;
			RayText();
		}
		else {
...
void AMyProjectCharacter::Interact() {
	IsInteracting = true;
	AActor* actor = hitRecord.GetActor();
	if (Cast<AMyDoor>(actor)) {
		auto* temp = Cast<AMyDoor>(actor);
		if (!temp->IsOpen) {
			if (hitRecord.Component->GetName() == "Handle" && !temp->IsLocked) {
				temp->PeekDoor();
			}
			else if (hitRecord.Component->GetName() == "Mesh") {
				// float x = GetWorld()->GetFirstPlayerController()->GetInputKeyTimeDown(FKey("E"));
				// GEngine->AddOnScreenDebugMessage(-1, 1.f, FColor::Red, FString::SanitizeFloat(x));
				if (temp->IsLocked && InteractionTimer >= 3.f) {
					temp->OpenDoor();
				}
...
void AMyProjectCharacter::RayText() {
	AActor* actor = hitRecord.GetActor();
	if (Cast<AMyDoor>(actor)) {
		auto* temp = Cast<AMyDoor>(actor);
		if (!temp->IsOpen) {
			if (hitRecord.Component->GetName() == "Handle" && !temp->IsLocked) {
				...
				if (temp->IsLocked) {
					if (InteractionTimer >= 3.f) {
						temp->OpenDoor();
						InteractText = "Unlocked";
					} else if (InteractionTimer < 3.f && InteractionTimer >0.f && IsInteracting) {
						InteractText = "Lock picking";
					} else {
...
void AMyProjectCharacter::CutInteractTimer() {
	IsInteracting = false;
	...
	if (ProjectileClass != nullptr) {
		...
		if (World != nullptr) {
			if (bUsingMotionControllers) {
				...
				World->SpawnActor<
					AMyProjectProjectile>(ProjectileClass, SpawnLocation, SpawnRotation, ActorSpawnParams);
			}
		}
	}
